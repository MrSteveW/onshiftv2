import { CreateDutyProps, Duty } from '@/components/types';
...
export default function AssigningOneDuty({ date, staff, tasks }: CreateDutyProps) {
    ...
    return (
        <>
            ...
            <div className="flex items-center justify-start gap-x-6">
                ...
                <a href="/staff" className="text-sm/6 font-semibold text-white">
                    Cancel
                </a>
            </div>
                ...
                <div className="flex gap-8 p-4">
                    ...
                    <AvailableColumn
                        duties={duties
                            ...
                            .map((duty) => ({
                                ...duty,
                                staffmember: staff?.find((s) => s.id === duty.staffmember_id)!,
                                task: null as any,
                            }))}
                    />
                    ...
                    {/* Task Slots */}
                    {tasks?.map((task) => {
                        const assignedDuty = duties.find((duty) => duty.task_id === task.id);
                        const dutyWithStaffmember = assignedDuty
                            ? {
                                  ...assignedDuty,
                                  staffmember: staff?.find((s) => s.id === assignedDuty.staffmember_id)!,
                                  task: task,
                              }
                            : null;
                        return <TaskSlot key={task.id} task={task} duty={dutyWithStaffmember} />;
                    })}
                </div>
            </DndContext>
        </>
